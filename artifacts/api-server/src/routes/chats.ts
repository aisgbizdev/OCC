...
router.post("/chats", authMiddleware, requireRole(...ALL_ROLES), async (req, res) => {
  try {
    ...
    if (chatType === "group" && !["Owner", "Chief Dealing", "SPV Dealing", "Corporate", "Co-SPV Dealing", "Admin System"].includes(req.user!.roleName)) {
...
router.post("/chats/:id/members", authMiddleware, requireRole("Owner", "Chief Dealing", "SPV Dealing", "Corporate", "Co-SPV Dealing", "Admin System"), async (req, res) => {
  try {
